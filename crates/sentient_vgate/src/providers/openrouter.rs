//! ─── OPENROUTER SAĞLAYICISI ───
//!
//! OpenRouter API ile iletişimi sağlayan sağlayıcı.
//! Birden fazla LLM modeline tek API ile erişim sağlar.
...
impl OpenRouterProvider {
    pub fn new(base_url: String, api_key: String) -> Self {
        let client = Client::builder()
            ...
            .build()
            .expect("OpenRouter: HTTP istemci oluşturulamadı");

        log::info!("🌐  OPENROUTER: Sağlayıcı başlatıldı → {}", base_url);
...
impl LlmProvider for OpenRouterProvider {
    async fn chat_completion(&self, request: LlmRequest) -> SENTIENTResult<LlmResponse> {
        ...
        log::debug!("🌐  OPENROUTER: İstek gönderiliyor → {}", request.model);
        ...
        if !status.is_success() {
            ...
            log::error!("🌐  OPENROUTER API Hatası [{}]: {}", status, error_text);
        ...
        let response_text = response.text().await.map_err(|e| {
            SENTIENTError::VGate(format!("Yanıt okuma hatası: {}", e))
        })?;

        let llm_response: LlmResponse = serde_json::from_str(&response_text).map_err(|e| {
            log::error!("🌐  OPENROUTER JSON hatası: {} | Yanıt: {}", e, 
                response_text.chars().take(500).collect::<String>());
            SENTIENTError::VGate(format!("JSON ayrıştırma hatası: {}", e))
        })?;

        log::info!(
            "🌐  OPENROUTER: Yanıt alındı → {} token",
    ...
    async fn list_models(&self) -> SENTIENTResult<Vec<ModelInfo>> {
                ...
                arr.iter()
                    .filter_map(|m| {
                        Some(ModelInfo {
                            ...
                            name: m["name"].as_str().unwrap_or("Bilinmeyen").to_string(),
        ...
        log::info!("🌐  OPENROUTER: {} model bulundu", models.len());
        Ok(models)
    }
