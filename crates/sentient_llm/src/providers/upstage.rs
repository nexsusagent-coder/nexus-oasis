...
/// Upstage provider
pub struct UpstageProvider {
    client: Client,
    api_key: String,
    base_url: String,
}

impl UpstageProvider {
    ...
    pub fn from_env() -> LlmResult<Self> {
        let api_key = std::env::var("UPSTAGE_API_KEY")
            .map_err(|_| LlmError::Authentication("UPSTAGE_API_KEY not set".into()))?;
        Self::new(api_key)
    }
}

#[async_trait]
impl LlmProvider for UpstageProvider {
    fn name(&self) -> &str { "Upstage" }
    fn id(&self) -> &str { "upstage" }

    fn models(&self) -> Vec<ModelInfo> {
        vec![
            ModelInfo {
                id: "solar-pro2-preview".into(),
                name: "Solar Pro 2 Preview".into(),
                provider: "Upstage".into(),
...
mod tests {
    use super::*;
    #[test]
    fn test_upstage_provider() { assert!(UpstageProvider::new("test-key").is_ok()); }
}
