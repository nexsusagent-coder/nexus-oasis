//! ─── OctoAI Provider ───
//!
//! OctoAI - Efficient cloud inference for open source models
//! https://octoai.ai
...
impl OctoAIProvider {
    ...
    pub fn from_env() -> LlmResult<Self> {
        let api_key = std::env::var("OCTOAI_TOKEN")
            .map_err(|_| LlmError::Authentication("OCTOAI_TOKEN not set".into()))?;
        Self::new(api_key)
    }
}

#[async_trait]
impl LlmProvider for OctoAIProvider {
    fn name(&self) -> &str { "OctoAI" }
    fn id(&self) -> &str { "octoai" }

    fn models(&self) -> Vec<ModelInfo> {
        vec![
            ModelInfo {
                id: "meta-llama/Meta-Llama-3.1-70B-Instruct".into(),
                name: "Llama 3.1 70B (OctoAI)".into(),
                provider: "OctoAI".into(),
...
mod tests {
    use super::*;
    #[test]
    fn test_octoai_provider() { assert!(OctoAIProvider::new("test-key").is_ok()); }
}
