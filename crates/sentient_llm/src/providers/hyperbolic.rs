...
/// Hyperbolic provider - Decentralized GPU marketplace
...
impl HyperbolicProvider {
    ...
    pub fn from_env() -> LlmResult<Self> {
        let api_key = std::env::var("HYPERBOLIC_API_KEY")
            .map_err(|_| LlmError::Authentication("HYPERBOLIC_API_KEY not set".into()))?;
        Self::new(api_key)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_hyperbolic_provider() {
        let provider = HyperbolicProvider::new("test-key");
        assert!(provider.is_ok());
    }

    #[test]
    fn test_hyperbolic_models() {
        let provider = HyperbolicProvider::new("test-key").unwrap();
