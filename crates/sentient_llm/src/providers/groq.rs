//! ─── Groq Provider ───
//!
//! Implementation for Groq API (LPU - fastest inference!)
...
/// Groq provider - LPU inference, fastest!
...
impl GroqProvider {
    ...
    pub fn from_env() -> LlmResult<Self> {
        let api_key = std::env::var("GROQ_API_KEY")
            .map_err(|_| LlmError::Authentication("GROQ_API_KEY not set".into()))?;
        Self::new(api_key)
    }
}
...
impl LlmProvider for GroqProvider {
    ...
    fn id(&self) -> &str { "groq" }
    fn models(&self) -> Vec<ModelInfo> { models::groq_models() }
            ...
            .json(&serde_json::json!({
                ...
                "messages": request.messages.iter().map(|m| serde_json::json!({
                    "role": match m.role {
                        ...
                        crate::types::Role::Function => "function",
                    },
                    "content": m.content.as_text().unwrap_or(""),
                })).collect::<Vec<_>>(),
                "max_tokens": request.max_tokens,
                "temperature": request.temperature,
                "tools": request.tools.map(|tools| tools.iter().map(|t| serde_json::json!({
                    "type": t.tool_type,
                    "function": {
...
mod tests {
    ...
    #[test]
    fn test_groq_free_models() {
        let models = models::groq_models();
        assert!(models.iter().all(|m| m.free_tier || m.input_cost_per_1k == 0.0));
    }
}
