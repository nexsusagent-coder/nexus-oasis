//! ─── Perplexity Provider ───
//!
//! Implementation for Perplexity API (online LLMs with web search)
...
/// Perplexity provider - online LLMs with web search!
...
impl PerplexityProvider {
    ...
    pub fn from_env() -> LlmResult<Self> {
        let api_key = std::env::var("PERPLEXITY_API_KEY")
            .map_err(|_| LlmError::Authentication("PERPLEXITY_API_KEY not set".into()))?;
        Self::new(api_key)
    }
}
...
impl LlmProvider for PerplexityProvider {
    ...
    fn models(&self) -> Vec<ModelInfo> { models::perplexity_models() }
