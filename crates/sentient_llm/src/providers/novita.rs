...
/// Novita AI provider - Cheap LLM inference
...
impl NovitaProvider {
    ...
    pub fn from_env() -> LlmResult<Self> {
        let api_key = std::env::var("NOVITA_API_KEY")
            .map_err(|_| LlmError::Authentication("NOVITA_API_KEY not set".into()))?;
        Self::new(api_key)
    }
}
...
impl LlmProvider for NovitaProvider {
    ...
    fn models(&self) -> Vec<ModelInfo> {
        vec![
            ...
            ModelInfo {
                id: "qwen/qwen-2.5-14b-instruct".into(),
            ...
            // ═══════════════════════════════════════════════════════════
            ModelInfo {
                id: "mistralai/mistral-7b-instruct".into(),
...
mod tests {
    ...
    #[test]
    fn test_novita_provider() {
        let provider = NovitaProvider::new("test-key");
        assert!(provider.is_ok());
    }

    #[test]
    fn test_novita_models() {
        let provider = NovitaProvider::new("test-key").unwrap();
