...
impl ConversationMemory {
    ...
    pub async fn store_turn(&self, entry: ConversationEntry) -> Result<()> {
        // Generate embedding (sync in this implementation)
        let embedding = self.embeddings.embed(&entry.content).ok();
    ...
    pub async fn get_history(&self, conversation_id: &str, limit: usize) -> Result<Vec<ConversationEntry>> {
        let results = self.memory.list(limit, 0).await?;
        
        Ok(results.into_iter()
            .filter_map(|entry| {
                let meta = entry.metadata.as_object()?;
                if meta.get("conversation_id")?.as_str()? != conversation_id {
                    return None;
                }
                ...
                let content = entry.content.strip_prefix(&format!("{}: ", role))
